...
class ASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        x4 = self.aspp4_bn(x4)
        x4 = self.relu(x4)
        x5 = self.global_pooling(x) #x5[1, 2048, 1, 1]
        x5 = self.aspp5(x5) #x5[1, 256, 1, 1]
        x5 = self.aspp5_bn(x5)
        x5 = self.relu(x5)
        x5 = nn.Upsample((x.shape[2], x.shape[3]), mode='bilinear',
                         align_corners=True)(x5)    #x5[1, 256, 33, 33]
        x = torch.cat((x1, x2, x3, x4, x5), 1)
        x = self.conv2(x)
        x = self.bn2(x)     #[1, 256, 33, 33]
        x = self.relu(x)
        x = self.conv3(x)   #[1, 21, 33, 33]
...
class ResNet(nn.Module):
    ...
    def forward(self, x):   #x: [1, 3, 513, 513]
        size = (x.shape[2], x.shape[3])
        x = self.conv1(x)   #[1, 64, 257, 257]
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x) #x: [1, 64, 129, 129]

        x = self.layer1(x)
        x = self.layer2(x)  #[1, 512, 65, 65]
        x = self.layer3(x)  #[1, 1024, 33, 33]
        x = self.layer4(x)  #[1, 2048, 33, 33]

        x = self.aspp(x)    #[1, 21, 33, 33]
        x = nn.Upsample(size, mode='bilinear', align_corners=True)(x)   #[1, 21, 513, 513]
        return x
